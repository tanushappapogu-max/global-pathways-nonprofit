...
WHERE college_type IN ('ivy_league', 't20', 'liberal_arts', 'technical') 
  ...
  AND (tuition_in_state IS NULL OR tuition_in_state = 0);

-- Add constraint to ensure data integrity
ALTER TABLE public.colleges_database_2025_10_06_01_15 
ADD CONSTRAINT check_tuition_logic 
CHECK (
  (college_type = 'state_school' AND tuition_in_state <= tuition_out_state) OR
  (college_type != 'state_school' AND tuition_in_state = tuition_out_state)
);
