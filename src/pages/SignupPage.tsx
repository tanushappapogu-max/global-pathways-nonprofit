  ...
  const googleLogin = useGoogleLogin({
    onSuccess: async (tokenResponse) => {
      ...
      try {
        ...
        // For now, we'll use the existing signUp function with Google data
        const { error, data } = await signUp(userInfo.email, 'google-oauth-' + userInfo.sub, {
    ...
    try {
      ...
      if (error) {
        ...
        toast({
          ...
          description: "Welcome to Global Pathways!",
        });
        setTimeout(() => navigate('/dashboard'), 500);
      } else {
        toast({
    ...
    <div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 flex items-center justify-center p-4">
      <Card className="w-full max-w-md bg-white/90 backdrop-blur-sm shadow-xl">
        <CardHeader className="text-center">
          ...
          <CardDescription>Join Global Pathways today</CardDescription>
        </CardHeader>
        <CardContent>
